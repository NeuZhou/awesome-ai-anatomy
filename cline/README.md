...
- [The God Object: Task (3,756 lines)](#the-god-object-task-3756-lines)
...
The fundamental issue is that these layers are coupled through callback injection rather than clean interfaces. The `Task` constructor takes 20+ parameters. The `ToolExecutor` constructor takes 30+ parameters including 15 callback functions like `saveCheckpoint`, `sayAndCreateMissingParamError`, `removeLastPartialMessageIfExistsWithType`, and `switchToActModeCallback`. This is constructor injection taken to its logical extreme — it works, it's testable in theory, but in practice it means any change to the communication protocol between classes requires touching half the codebase.
...
The recursive nature is notable: `recursivelyMakeClineRequests` calls itself with tool results. This is recursive in the call stack, not just conceptually — deep tool chains produce deep recursion. There's no explicit tail-call optimization or iterative rewrite. In practice, the stack depth is bounded by the context window running out, but it's architecturally fragile.
...
**The risk:** Hook scripts run with the same permissions as VS Code. A malicious `.cline/hooks/` directory in a cloned repo could execute arbitrary code when the user opens the project. The `HookDiscoveryCache` caches discovered hooks for performance, and there's a `hooksEnabled` global setting, but there's no sandboxing or signature verification.

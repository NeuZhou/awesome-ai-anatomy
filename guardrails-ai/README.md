...
One detail: the validation service defaults to async execution. `validator_service/__init__.py` tries to grab an event loop and use `AsyncValidatorService` first, falling back to `SequentialValidatorService` only if that fails. Most users probably don't realize their validators are running in an async context by default.

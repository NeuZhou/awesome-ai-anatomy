...
graph TB
    subgraph Entry["🚀 Entry Layer"]
        CLI["codex CLI<br/>(Clap parser)"]
        MCP_ENTRY["codex mcp<br/>(MCP server mode)"]
        APP["codex app<br/>(Desktop app)"]
    end

    subgraph Server["🖥️ App Server Layer"]
        CLI --> TUI["TUI<br/>(Ratatui terminal UI)"]
        TUI --> AS["App Server<br/>(JSON-RPC over stdio/WS)"]
        APP --> AS
        MCP_ENTRY --> MCPS["MCP Server<br/>(rmcp protocol)"]
    end

    subgraph Core["⚡ Core Engine (codex-core)"]
        AS --> CODEX["Codex struct<br/>(queue pair: submit/event)"]
        MCPS --> CODEX
        CODEX --> CTX["Context Manager<br/>(token tracking + truncation)"]
        CODEX --> CLIENT["Model Client<br/>(SSE + WebSocket streaming)"]
        CODEX --> TOOLS["Tool Router<br/>(registry + dispatch)"]
        CODEX --> COMPACT["Compaction<br/>(inline + remote)"]
        CODEX --> GUARD["Guardian<br/>(AI auto-approvals)"]
        CODEX --> MEM["Memories<br/>(2-phase extraction)"]
        CODEX --> SKILLS_CORE["Skills Manager"]
        CODEX --> HOOKS_CORE["Hook Runtime"]
        CODEX --> PLUGINS["Plugins Manager"]
    end

    subgraph ToolExec["🔧 Tool Execution"]
        TOOLS --> ORCH["Tool Orchestrator<br/>(approval → sandbox → retry)"]
        ORCH --> SHELL_RT["Shell Runtime"]
        ORCH --> PATCH_RT["Apply Patch Runtime"]
        ORCH --> AGENT_RT["Sub-Agent Spawn"]
        ORCH --> MCP_TOOL["MCP Tool Calls"]
        ORCH --> JS_REPL["JS REPL Kernel"]
    end

    subgraph Sandbox["🔒 Sandbox Layer"]
        SHELL_RT --> SB["Sandbox Manager"]
        SB --> SEAT["Seatbelt<br/>(macOS sandbox-exec)"]
        SB --> LAND["Landlock + bwrap + seccomp<br/>(Linux)"]
        SB --> WINBOX["Restricted Token + ACL<br/>(Windows)"]
    end

    subgraph Network["🌐 Network Control"]
        CODEX --> NP["Network Proxy<br/>(MITM HTTP/SOCKS5)"]
        NP --> POLICY["Network Policy<br/>(domain allowlist)"]
    end

    subgraph Infra["📊 Infrastructure"]
        CODEX --> FEAT["Feature Flags<br/>(88+ flags)"]
        CODEX --> STATE["State DB<br/>(SQLite rollouts)"]
        CODEX --> OTEL["OpenTelemetry"]
        CODEX --> AUTH["Auth Manager<br/>(OAuth + API Key)"]
    end
